...
export default function Coleção(props) {
  ...
  async function handleDelete(arg) {
    try {
      await axios.delete(`https://ironrest.herokuapp.com/abenete/${arg}`);
      setRerender(true);
    } catch (error) {
      console.error(error);
    }
                            ...
                            <div className='ml-4'>
                              <div className='mt-1 font-serif '>
                                {ref.author
                                  ...
                                  .join(" ")
                                  .toUpperCase()}
                                , {ref.author.split(" ").slice(0, -1).join(" ")}
                                .{` ${ref.title.toUpperCase()}`}
                                {ref.subtitle.length !== 0 ? (
                                  `: ${ref.subtitle}`
                                ) : (
                                  <></>
                                )}
                                .{" "}
                                <span className='font-bold'>{ref.revista}</span>
                                ,{` ${ref.local}`}, v. {ref.volume}, n.{" "}
                                {ref.fascicle}, p. {ref.initialPage}-
                                {ref.finalPage},{` ${mesConvertido(ref.month)}`}
                                . {ref.year}.
                              </div>
                            </div>
                          </div>
